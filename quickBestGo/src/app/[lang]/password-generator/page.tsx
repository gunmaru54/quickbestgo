...
export async function generateMetadata({ params: { lang } }: { params: { lang: Locale } }): Promise<Metadata> {
  ...
  return constructMetadata({ title: dict.password_generator.meta_title, description: dict.password_generator.meta_description, lang, slug: 'password-generator' });
}

export default async function PasswordGeneratorPage({ params: { lang } }: { params: { lang: Locale } }) {
  const dict = await getDictionary(lang);
  const d = dict.password_generator;
  const schemas = generateWebApplicationSchema({ name: d.title, description: d.meta_description, lang, slug: 'password-generator', category: 'SecurityApplication' });
  const toolConfig = TOOLS.find(t => t.slug === 'password-generator')!;
  ...
  return (
    <ToolPageTemplate
      ...
      toolComponent={<PasswordGenerator dict={d} theme={theme} />}
