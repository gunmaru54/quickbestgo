...
export async function generateMetadata({ params: { lang } }: { params: { lang: Locale } }): Promise<Metadata> {
  ...
  return constructMetadata({
    title: dict.terms_of_service_page.meta_title,
    description: dict.terms_of_service_page.meta_description,
    lang,
...
export default async function TermsOfServicePage({ params: { lang } }: { params: { lang: Locale } }) {
  const dict = await getDictionary(lang);
  const p = dict.terms_of_service_page;
  ...
  const sections = [
    ...
    { title: p.s7_title, content: p.s7_content },
